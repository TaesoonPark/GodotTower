...
const REPATH_INTERVAL_SEC: float = 0.85
const SEARCH_MARGIN_TILES: int = 12
const MAX_EXPANSIONS: int = 200
...
func get_debug_snapshot() -> Dictionary:
	return {
		...
		"last_goal": _last_path_goal,
		"repath_left": _repath_left,
		"stuck_elapsed": _stuck_elapsed,
		"tile_size": tile_size
	}
...
func move_step(current_pos: Vector2, goal_world: Vector2, speed: float, delta: float, is_blocked: Callable) -> Dictionary:
	...
	if _path_points.is_empty():
		...
		var escape_step: Vector2 = _find_escape_step(current_pos, speed, safe_delta, is_blocked)
		if escape_step != Vector2.INF:
			return _set_result(escape_step, false, false)
...
func _find_escape_step(current_pos: Vector2, speed: float, safe_delta: float, is_blocked: Callable) -> Vector2:
	...
	for d in dirs:
		var candidate: Vector2 = current_pos + d * speed * safe_delta
		if not bool(is_blocked.call(candidate)):
			return candidate
	return Vector2.INF
