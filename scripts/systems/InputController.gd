...
func process_unhandled_input(event: InputEvent, world: Node2D) -> void:
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT:
		if event.pressed:
			dragging = true
			drag_start = _snap_to_grid(world.get_global_mouse_position())
			drag_end = drag_start
		else:
			if dragging and drag_start.distance_to(drag_end) < 12.0:
				left_click.emit(_snap_to_grid(world.get_global_mouse_position()))
			else:
				drag_selection.emit(drag_start, drag_end)
			dragging = false
	if event is InputEventMouseMotion and dragging:
		drag_end = _snap_to_grid(world.get_global_mouse_position())
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_RIGHT and event.pressed:
		command_move.emit(_snap_to_grid(world.get_global_mouse_position()))
