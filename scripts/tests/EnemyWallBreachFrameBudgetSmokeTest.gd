extends Node

const MAIN_SCENE: PackedScene = preload("res://scenes/main/Main.tscn")
const RAIDER_SCENE: PackedScene = preload("res://scenes/units/Raider.tscn")
const EXIT_PASS: int = 0
const EXIT_FAIL: int = 1
const ENEMY_COUNT: int = 64
const FRAME_BUDGET_USEC: int = 100000
const ENEMY_AI_BUDGET_MS: float = 85.0

func _ready() -> void:
	call_deferred("_run_test")

func _finish(success: bool, message: String) -> void:
	if success:
		print(message)
		get_tree().quit(EXIT_PASS)
		return
	printerr(message)
	get_tree().quit(EXIT_FAIL)

func _run_test() -> void:
	OS.set_environment("PERF_LOGS", "1")
	var main = MAIN_SCENE.instantiate()
	add_child(main)
	for _i in range(24):
		await get_tree().process_frame

	var colonists: Array = get_tree().get_nodes_in_group("colonists")
	if colonists.is_empty():
		_finish(false, "ENEMY_WALL_BREACH_FRAME_BUDGET_FAIL: missing colonist")
		return
	var target: Node2D = colonists[0] as Node2D
	var inside: Vector2 = Vector2(3840.0, 2160.0)
	target.global_position = inside
	target.set("health", 100000.0)
	if target.has_method("cancel_current_job"):
		target.cancel_current_job()
	for i in range(1, colonists.size()):
		var colonist: Node2D = colonists[i] as Node2D
		if colonist == null or not is_instance_valid(colonist):
			continue
		colonist.global_position = Vector2(4480.0, 1840.0 + float(i) * 80.0)
		colonist.set("health", 100000.0)
		if colonist.has_method("cancel_current_job"):
			colonist.cancel_current_job()

	_place_wall_box(main, inside)
	_prepare_breachable_wall(inside + Vector2(-80.0, 0.0))
	main._mark_pathing_dirty()
	main._dispatch_event_updates()
	for _i in range(4):
		await get_tree().process_frame

	var enemies: Array[Node2D] = _spawn_enemies(main, target, inside + Vector2(-320.0, 0.0))
	main._raid_state = &"Active"
	main._cached_alive_enemies = enemies
	main._mark_combat_dirty()
	main._set_game_speed(4.0)

	var breached: bool = false
	for _i in range(120):
		await get_tree().process_frame
		if not _wall_exists_at(inside + Vector2(-80.0, 0.0)):
			breached = true
			break
	if not breached:
		_finish(false, "ENEMY_WALL_BREACH_FRAME_BUDGET_FAIL: wall did not breach")
		return

	EnemyUnitBase.consume_perf_stats()
	var max_frame_us: int = 0
	for _i in range(120):
		var frame_start_us: int = Time.get_ticks_usec()
		await get_tree().process_frame
		max_frame_us = maxi(max_frame_us, Time.get_ticks_usec() - frame_start_us)
	var enemy_perf: Dictionary = EnemyUnitBase.consume_perf_stats()
	var enemy_ai_ms: float = float(enemy_perf.get("ai_ms", 0.0))
	if max_frame_us > FRAME_BUDGET_USEC:
		_finish(false, "ENEMY_WALL_BREACH_FRAME_BUDGET_FAIL: max frame took %dus after breach" % max_frame_us)
		return
	if enemy_ai_ms > ENEMY_AI_BUDGET_MS:
		_finish(false, "ENEMY_WALL_BREACH_FRAME_BUDGET_FAIL: enemy ai %.2fms after breach" % enemy_ai_ms)
		return

	_finish(true, "ENEMY_WALL_BREACH_FRAME_BUDGET_PASS: post-breach movement stayed within frame budget")

func _place_wall_box(main: Node, center: Vector2) -> void:
	main.build_system.set_selected_building(&"Wall")
	for x in range(int(center.x - 80.0), int(center.x + 81.0), 40):
		main.build_system.place_building(Vector2(float(x), center.y - 80.0), false)
		main.build_system.place_building(Vector2(float(x), center.y + 80.0), false)
	for y in range(int(center.y - 40.0), int(center.y + 41.0), 40):
		main.build_system.place_building(Vector2(center.x - 80.0, float(y)), false)
		main.build_system.place_building(Vector2(center.x + 80.0, float(y)), false)

func _prepare_breachable_wall(weak_wall_pos: Vector2) -> void:
	for structure in get_tree().get_nodes_in_group("structures"):
		if structure == null or not is_instance_valid(structure):
			continue
		if not structure.has_meta("building_id") or StringName(structure.get_meta("building_id")) != &"Wall":
			continue
		if (structure as Node2D).global_position.distance_to(weak_wall_pos) <= 0.1:
			structure.set_meta("structure_max_health", 40.0)
			structure.set_meta("structure_health", 40.0)
		else:
			structure.set_meta("structure_max_health", 100000.0)
			structure.set_meta("structure_health", 100000.0)

func _wall_exists_at(pos: Vector2) -> bool:
	for structure in get_tree().get_nodes_in_group("structures"):
		if structure == null or not is_instance_valid(structure):
			continue
		if not structure.has_meta("building_id") or StringName(structure.get_meta("building_id")) != &"Wall":
			continue
		if (structure as Node2D).global_position.distance_to(pos) <= 0.1:
			return true
	return false

func _spawn_enemies(main: Node, target: Node2D, center: Vector2) -> Array[Node2D]:
	var enemies: Array[Node2D] = []
	var offsets: Array[Vector2] = _build_offsets(ENEMY_COUNT, 40.0)
	for i in range(ENEMY_COUNT):
		var enemy: Node2D = RAIDER_SCENE.instantiate()
		enemy.global_position = main._snap_to_tile(center + offsets[i])
		if enemy.has_method("set_tile_size"):
			enemy.set_tile_size(64.0)
		main.units_root.add_child(enemy)
		enemy.set("_target_colonist_id", target.get_instance_id())
		enemy.set("_target_refresh_left", 0.0)
		enemy.set("_spawn_unclip_left", 0.0)
		enemy.set("structure_attack_damage", 80.0)
		enemy.set("attack_cooldown_sec", 0.2)
		enemies.append(enemy)
	return enemies

func _build_offsets(count: int, next_tile_size: float) -> Array[Vector2]:
	var offsets: Array[Vector2] = []
	for y in range(-4, 5):
		for x in range(-4, 4):
			offsets.append(Vector2(float(x) * next_tile_size, float(y) * next_tile_size))
			if offsets.size() >= count:
				return offsets
	return offsets
