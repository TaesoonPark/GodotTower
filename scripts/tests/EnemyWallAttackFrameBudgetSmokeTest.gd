...
const ENEMY_AI_BUDGET_MS: float = 90.0
...
func _run_test() -> void:
	...
	if colonists.is_empty():
		_finish(false, "ENEMY_WALL_ATTACK_FRAME_BUDGET_FAIL: missing colonist")
		return
	...
	if max_frame_us > FRAME_BUDGET_USEC:
		_finish(false, "ENEMY_WALL_ATTACK_FRAME_BUDGET_FAIL: max frame took %dus" % max_frame_us)
		return
	if enemy_ai_ms > ENEMY_AI_BUDGET_MS:
		_finish(false, "ENEMY_WALL_ATTACK_FRAME_BUDGET_FAIL: enemy ai %.2fms" % enemy_ai_ms)
		return

	_finish(true, "ENEMY_WALL_ATTACK_FRAME_BUDGET_PASS: enclosed target wall attack stayed within frame budget")
