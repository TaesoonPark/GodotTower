...
const ENEMY_AI_BUDGET_MS: float = 80.0
const COORDINATOR_BUDGET_MS: float = 80.0
...
func _run_test() -> void:
	...
	if colonists.is_empty():
		_finish(false, "ENEMY_TARGET_EXIT_FRAME_BUDGET_FAIL: missing colonist")
		return
	...
	var coord_before: Dictionary = coordinator.get_debug_stats() if coordinator != null and is_instance_valid(coordinator) and coordinator.has_method("get_debug_stats") else {}
	...
	var coord_after: Dictionary = coordinator.get_debug_stats() if coordinator != null and is_instance_valid(coordinator) and coordinator.has_method("get_debug_stats") else {}
	var coord_ms: float = float(coord_after.get("total_request_ms", 0.0)) - float(coord_before.get("total_request_ms", 0.0))
	var enemy_ai_ms: float = float(enemy_perf.get("ai_ms", 0.0))
	if max_frame_us > FRAME_BUDGET_USEC:
		_finish(false, "ENEMY_TARGET_EXIT_FRAME_BUDGET_FAIL: max frame took %dus after target exit" % max_frame_us)
		return
	if enemy_ai_ms > ENEMY_AI_BUDGET_MS:
		_finish(false, "ENEMY_TARGET_EXIT_FRAME_BUDGET_FAIL: enemy ai %.2fms" % enemy_ai_ms)
		return
	if coord_ms > COORDINATOR_BUDGET_MS:
		_finish(false, "ENEMY_TARGET_EXIT_FRAME_BUDGET_FAIL: coordinator %.2fms" % coord_ms)
		return

	_finish(true, "ENEMY_TARGET_EXIT_FRAME_BUDGET_PASS: target exit stayed inside frame and enemy AI budgets")
